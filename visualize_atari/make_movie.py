...
def make_movie(env_name, checkpoint='*.tar', num_frames=20, first_frame=0, resolution=75, \
    ...
    movie_title = "{}-{}-{}-Greydanus.mp4".format(prefix, num_frames, env_name.lower())
    movie_title_f = "{}-{}-{}-f-metric.mp4".format(prefix, num_frames, env_name.lower())
    ...
    start = time.time()
    FFMpegWriter = manimation.writers['ffmpeg']
    metadata = dict(title=movie_title_f)
    writer = FFMpegWriter(fps=8, metadata=metadata)

    prog = '' ; total_frames = len(history['ins'])
    f = plt.figure(figsize=[6, 6*1.3], dpi=resolution)
    with writer.saving(f, save_dir + movie_title_f, resolution):
        for i in range(num_frames):
            ix = first_frame+i
            if ix < total_frames: # prevent loop from trying to process a frame ix greater than rollout length
                frame_f = history['ins'][ix].squeeze().copy()
